...
class TestCatalogSession(object):
    """Tests for CatalogSession"""
    def test_can_lookup_mappings(self):
        """Tests can_lookup_mappings"""
        # From test_templates/resource.py::ResourceBinSession::can_lookup_resource_bin_mappings
        result = self.session.can_lookup_mappings()
    ...
    def test_use_federated_catalog_view(self):
        """Tests use_federated_catalog_view"""
        # From test_templates/resource.py ResourceLookupSession.use_federated_bin_view_template
        self.catalog.use_federated_catalog_view()

    def test_use_isolated_catalog_view(self):
        """Tests use_isolated_catalog_view"""
        # From test_templates/resource.py ResourceLookupSession.use_isolated_bin_view_template
        self.catalog.use_isolated_catalog_view()

    def test_get_ids_by_catalog(self):
        ...
        if not is_never_authz(self.service_config):
            ...
            with pytest.raises(errors.PermissionDenied):
                self.svc_mgr.get_ids_by_catalog(self.fake_id)

    def test_get_ids_by_catalogs(self):
        """Tests get_ids_by_catalogs"""
        ...
        if not is_never_authz(self.service_config):
            ...
            object_ids = self.session.get_ids_by_catalogs(catalog_ids)
            assert isinstance(object_ids, IdList)
            # Currently our impl does not remove duplicate objectIds
            assert object_ids.available() == 5
        else:
            with pytest.raises(errors.PermissionDenied):
                self.session.get_ids_by_catalogs([self.fake_id])

    def test_get_catalog_ids_by_id(self):
        """Tests get_catalog_ids_by_id"""
        # From test_templates/resource.py::ResourceBinSession::get_bin_ids_by_resource_template
        if not is_never_authz(self.service_config):
            cats = self.svc_mgr.get_catalog_ids_by_id(self.id_ids[1])
            assert cats.available() == 2
        else:
            with pytest.raises(errors.PermissionDenied):
                self.svc_mgr.get_catalog_ids_by_id(self.fake_id)

    def test_get_catalogs_by_id(self):
        """Tests get_catalogs_by_id"""
        # From test_templates/resource.py::ResourceBinSession::get_bins_by_resource_template
        if not is_never_authz(self.service_config):
            cats = self.svc_mgr.get_catalogs_by_id(self.id_ids[1])
            assert cats.available() == 2
        else:
            with pytest.raises(errors.PermissionDenied):
...
class TestCatalogAssignmentSession(object):
    ...
    def test_can_assign_catalogs(self):
        """Tests can_assign_catalogs"""
        # From test_templates/resource.py::ResourceBinAssignmentSession::can_assign_resources_template
        result = self.session.can_assign_catalogs()
        assert isinstance(result, bool)
    ...
    def test_reassign_id_to_catalog(self):
        ...
        if is_never_authz(self.service_config):
            ...
            with pytest.raises(errors.Unimplemented):
                self.session.reassign_id_to_catalog(True, True, True)
...
class TestCatalogAdminSession(object):
    ...
    def test_get_catalog_form_for_create(self):
        ...
        if not is_never_authz(self.service_config):
            ...
            assert isinstance(catalog_form, CatalogForm)
...
class TestCatalogHierarchySession(object):
    ...
    def test_get_root_catalogs(self):
        ...
        if not is_never_authz(self.service_config):
            ...
            assert isinstance(roots, CatalogList)
...
class TestCatalogHierarchyDesignSession(object):
    ...
    def test_add_root_catalog(self):
        ...
        if not is_never_authz(self.service_config):
            ...
            assert isinstance(roots, ABCObjects.CatalogList)
            assert roots.available() == 1
        else:
            with pytest.raises(errors.PermissionDenied):
    ...
    def test_add_child_catalog(self):
        ...
        if not is_never_authz(self.service_config):
            ...
            assert isinstance(children, ABCObjects.CatalogList)
