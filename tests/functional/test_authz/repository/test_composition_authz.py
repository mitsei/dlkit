"""TestAuthZ implementations of repository.Composition"""
...
LOOKUP_COMPOSITION_FUNCTION_ID = Id(**{'identifier': 'lookup', 'namespace': 'repository.Composition', 'authority': 'ODL.MIT.EDU'})
SEARCH_COMPOSITION_FUNCTION_ID = Id(**{'identifier': 'search', 'namespace': 'repository.Composition', 'authority': 'ODL.MIT.EDU'})
CREATE_COMPOSITION_FUNCTION_ID = Id(**{'identifier': 'create', 'namespace': 'repository.Composition', 'authority': 'ODL.MIT.EDU'})
DELETE_COMPOSITION_FUNCTION_ID = Id(**{'identifier': 'delete', 'namespace': 'repository.Composition', 'authority': 'ODL.MIT.EDU'})
ASSIGN_COMPOSITION_FUNCTION_ID = Id(**{'identifier': 'assign', 'namespace': 'repository.CompositionRepository', 'authority': 'ODL.MIT.EDU'})
...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        #       |        |        |
        #       3        4       5(t)      6     7(t)   (the 'blue' composition in repository 2 is also assigned to repository 7)
        ...
        # Set up Composition lookup authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                LOOKUP_COMPOSITION_FUNCTION_ID,
        ...
        # Set up Composition lookup override authorizations for Jane
        for num in [7]:
        ...
        # Set up Composition search override authorizations for Jane
        for num in [7]:
            create_form = request.cls.override_authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_COMPOSITION_FUNCTION_ID,
                request.cls.repository_id_list[num],
                [])
            create_form.display_name = 'Test Authorization ' + str(num) + ' (override)'
            create_form.description = 'Test Authorization for AuthorizationSession tests'
        ...
        # Set up Composition search authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_COMPOSITION_FUNCTION_ID,
...
class TestCompositionAuthzAdapter(object):
    ...
    def test_lookup_repository_0_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            # with pytest.raises(errors.NotFound):
            #     compositions = repository.get_compositions_by_genus_type(BLUE_TYPE)
            # for composition_id in self.composition_id_lists[0]:
            #     with pytest.raises(errors.NotFound):
            #         composition = repository.get_composition(composition_id)
            # with pytest.raises(errors.NotFound):
            #     compositions = repository.get_compositions_by_ids(self.composition_id_lists[0])
    ...
    def test_lookup_repository_0_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            repository.use_plenary_composition_view()
            assert repository.can_lookup_compositions()
            assert repository.get_compositions().available() == 1
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).available() == 1
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).next().ident == self.composition_id_lists[2][1]
            repository.get_composition(self.composition_id_lists[2][1])
    ...
    def test_lookup_repository_0_comparative_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            repository.use_isolated_repository_view()
            repository.use_comparative_composition_view()
            assert repository.get_compositions().available() == 0
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).available() == 0
    ...
    def test_lookup_repository_1_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            repository.use_isolated_repository_view()
            repository.use_plenary_composition_view()
            assert repository.get_compositions().available() == 3
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).available() == 1
    ...
    def test_lookup_repository_1_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            repository.use_federated_repository_view()
            repository.use_plenary_composition_view()
            assert repository.get_compositions().available() == 9
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).available() == 3
    ...
    def test_lookup_repository_2_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            repository.use_federated_repository_view()
            repository.use_comparative_composition_view()
            assert repository.get_compositions().available() == 4
            assert repository.get_compositions_by_genus_type(BLUE_TYPE).available() == 2
            # self.assertEqual(repository.get_compositions().available(), 3)
            # self.assertEqual(repository.get_compositions_by_genus_type(BLUE_TYPE).available(), 1)
    ...
    def test_query_repository_0_federated(self):
        if not is_never_authz(self.service_config):
            ...
            assert repository.get_compositions_by_query(query).available() == 8
            query.clear_display_name_terms()
            query.match_display_name('blue')
            assert repository.get_compositions_by_query(query).available() == 5
    ...
    def test_query_repository_1_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            query = repository.get_composition_query()
            query.match_display_name('red')
            assert repository.get_compositions_by_query(query).available() == 2
    ...
    def test_query_repository_1_federated(self):
        if not is_never_authz(self.service_config):
            ...
            query = repository.get_composition_query()
            query.match_display_name('red')
            assert repository.get_compositions_by_query(query).available() == 6
