"""TestAuthZ implementations of grading.GradeSystem"""
...
LOOKUP_GRADE_SYSTEM_FUNCTION_ID = Id(**{'identifier': 'lookup', 'namespace': 'grading.GradeSystem', 'authority': 'ODL.MIT.EDU'})
SEARCH_GRADE_SYSTEM_FUNCTION_ID = Id(**{'identifier': 'search', 'namespace': 'grading.GradeSystem', 'authority': 'ODL.MIT.EDU'})
CREATE_GRADE_SYSTEM_FUNCTION_ID = Id(**{'identifier': 'create', 'namespace': 'grading.GradeSystem', 'authority': 'ODL.MIT.EDU'})
DELETE_GRADE_SYSTEM_FUNCTION_ID = Id(**{'identifier': 'delete', 'namespace': 'grading.GradeSystem', 'authority': 'ODL.MIT.EDU'})
ASSIGN_GRADE_SYSTEM_FUNCTION_ID = Id(**{'identifier': 'assign', 'namespace': 'grading.GradeSystemGradebook', 'authority': 'ODL.MIT.EDU'})
...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        #       |        |        |
        #       3        4       5(t)      6     7(t)   (the 'blue' grade_system in gradebook 2 is also assigned to gradebook 7)
        ...
        # Set up GradeSystem lookup authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                LOOKUP_GRADE_SYSTEM_FUNCTION_ID,
        ...
        # Set up GradeSystem lookup override authorizations for Jane
        for num in [7]:
        ...
        # Set up GradeSystem search override authorizations for Jane
        for num in [7]:
            create_form = request.cls.override_authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_GRADE_SYSTEM_FUNCTION_ID,
                request.cls.gradebook_id_list[num],
                [])
            create_form.display_name = 'Test Authorization ' + str(num) + ' (override)'
            create_form.description = 'Test Authorization for AuthorizationSession tests'
        ...
        # Set up GradeSystem search authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_GRADE_SYSTEM_FUNCTION_ID,
...
def authz_adapter_test_fixture(request):
    request.cls.grade_system_id_lists = []
    count = 0
    if not is_never_authz(request.cls.service_config):
        for gradebook_ in request.cls.gradebook_list:
            request.cls.grade_system_id_lists.append([])
            for color in ['Red', 'Blue', 'Red']:
                create_form = gradebook_.get_grade_system_form_for_create([])
                create_form.display_name = color + ' ' + str(count) + ' GradeSystem'
                create_form.description = color + ' grade_system for authz adapter tests from Gradebook number ' + str(count)
                if color == 'Blue':
                    create_form.genus_type = BLUE_TYPE
                grade_system = gradebook_.create_grade_system(create_form)
                if count == 2 and color == 'Blue':
                    request.cls.grading_mgr.assign_grade_system_to_gradebook(
                        grade_system.ident,
                        request.cls.gradebook_id_list[7])
                request.cls.grade_system_id_lists[count].append(grade_system.ident)
            count += 1

    def test_tear_down():
        if not is_never_authz(request.cls.service_config):
            for index, gradebook_ in enumerate(request.cls.gradebook_list):
                for grade_system_id in request.cls.grade_system_id_lists[index]:
                    gradebook_.delete_grade_system(grade_system_id)

    request.addfinalizer(test_tear_down)


@pytest.mark.usefixtures("authz_adapter_class_fixture", "authz_adapter_test_fixture")
class TestGradeSystemAuthzAdapter(object):
    ...
    def test_lookup_gradebook_0_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            # with pytest.raises(errors.NotFound):
            #     grade_systems = gradebook.get_grade_systems_by_genus_type(BLUE_TYPE)
            # for grade_system_id in self.grade_system_id_lists[0]:
            #     with pytest.raises(errors.NotFound):
            #         grade_system = gradebook.get_grade_system(grade_system_id)
            # with pytest.raises(errors.NotFound):
            #     grade_systems = gradebook.get_grade_systems_by_ids(self.grade_system_id_lists[0])
    ...
    def test_lookup_gradebook_0_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_plenary_grade_system_view()
            assert gradebook.can_lookup_grade_systems()
            assert gradebook.get_grade_systems().available() == 1
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 1
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).next().ident == self.grade_system_id_lists[2][1]
            gradebook.get_grade_system(self.grade_system_id_lists[2][1])
            for grade_system_num in [0, 2]:
                with pytest.raises(errors.NotFound):  # Is this right?  Perhaps PermissionDenied
                    grade_system = gradebook.get_grade_system(self.grade_system_id_lists[2][grade_system_num])
    ...
    def test_lookup_gradebook_0_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            # print "START"
            assert gradebook.get_grade_systems().available() == 13
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 5
            for grade_system in gradebook.get_grade_systems():
                gradebook.get_grade_system(grade_system.ident)
            grade_system_ids = [grade_system.ident for grade_system in gradebook.get_grade_systems()]
            gradebook.get_grade_systems_by_ids(grade_system_ids)
            for grade_system_id in self.grade_system_id_lists[0]:
                with pytest.raises(errors.NotFound):
                    grade_system = gradebook.get_grade_system(grade_system_id)
            grade_system = gradebook.get_grade_system(self.grade_system_id_lists[2][1])
            for grade_system_num in [0, 2]:
                with pytest.raises(errors.NotFound):
                    grade_system = gradebook.get_grade_system(self.grade_system_id_lists[2][grade_system_num])
            for grade_system_id in self.grade_system_id_lists[1]:
                    grade_system = gradebook.get_grade_system(grade_system_id)
            for grade_system_id in self.grade_system_id_lists[3]:
                    grade_system = gradebook.get_grade_system(grade_system_id)
            for grade_system_id in self.grade_system_id_lists[4]:
                    grade_system = gradebook.get_grade_system(grade_system_id)
            for grade_system_id in self.grade_system_id_lists[5]:
                    grade_system = gradebook.get_grade_system(grade_system_id)
    ...
    def test_lookup_gradebook_0_comparative_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_isolated_gradebook_view()
            gradebook.use_comparative_grade_system_view()
            assert gradebook.get_grade_systems().available() == 0
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 0
    ...
    def test_lookup_gradebook_1_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_isolated_gradebook_view()
            gradebook.use_plenary_grade_system_view()
            assert gradebook.get_grade_systems().available() == 3
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 1
    ...
    def test_lookup_gradebook_1_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_federated_gradebook_view()
            gradebook.use_plenary_grade_system_view()
            assert gradebook.get_grade_systems().available() == 9
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 3
    ...
    def test_lookup_gradebook_2_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_federated_gradebook_view()
            gradebook.use_comparative_grade_system_view()
            assert gradebook.get_grade_systems().available() == 4
            assert gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available() == 2
            # self.assertEqual(gradebook.get_grade_systems().available(), 3)
            # self.assertEqual(gradebook.get_grade_systems_by_genus_type(BLUE_TYPE).available(), 1)
    ...
    def test_query_gradebook_0_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            gradebook.use_isolated_gradebook_view()
            with pytest.raises(errors.PermissionDenied):
                query = gradebook.get_grade_system_query()
    ...
    def test_query_gradebook_0_federated(self):
        if not is_never_authz(self.service_config):
            ...
            assert gradebook.get_grade_systems_by_query(query).available() == 8
            query.clear_display_name_terms()
            query.match_display_name('blue')
            assert gradebook.get_grade_systems_by_query(query).available() == 5
    ...
    def test_query_gradebook_1_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            query = gradebook.get_grade_system_query()
            query.match_display_name('red')
            assert gradebook.get_grade_systems_by_query(query).available() == 2
    ...
    def test_query_gradebook_1_federated(self):
        if not is_never_authz(self.service_config):
            ...
            query = gradebook.get_grade_system_query()
            query.match_display_name('red')
            assert gradebook.get_grade_systems_by_query(query).available() == 6
