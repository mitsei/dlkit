"""TestAuthZ implementations of resource.ResourceRelationship"""
...
LOOKUP_RESOURCE_RELATIONSHIP_FUNCTION_ID = Id(**{'identifier': 'lookup', 'namespace': 'resource.ResourceRelationship', 'authority': 'ODL.MIT.EDU'})
SEARCH_RESOURCE_RELATIONSHIP_FUNCTION_ID = Id(**{'identifier': 'search', 'namespace': 'resource.ResourceRelationship', 'authority': 'ODL.MIT.EDU'})
CREATE_RESOURCE_RELATIONSHIP_FUNCTION_ID = Id(**{'identifier': 'create', 'namespace': 'resource.ResourceRelationship', 'authority': 'ODL.MIT.EDU'})
DELETE_RESOURCE_RELATIONSHIP_FUNCTION_ID = Id(**{'identifier': 'delete', 'namespace': 'resource.ResourceRelationship', 'authority': 'ODL.MIT.EDU'})
ASSIGN_RESOURCE_RELATIONSHIP_FUNCTION_ID = Id(**{'identifier': 'assign', 'namespace': 'resource.ResourceRelationshipBin', 'authority': 'ODL.MIT.EDU'})
...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        #       |        |        |
        #       3        4       5(t)      6     7(t)   (the 'blue' resource_relationship in bin 2 is also assigned to bin 7)
        ...
        # Set up ResourceRelationship lookup authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                LOOKUP_RESOURCE_RELATIONSHIP_FUNCTION_ID,
        ...
        # Set up ResourceRelationship lookup override authorizations for Jane
        for num in [7]:
        ...
        # Set up ResourceRelationship search override authorizations for Jane
        for num in [7]:
            create_form = request.cls.override_authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_RESOURCE_RELATIONSHIP_FUNCTION_ID,
                request.cls.bin_id_list[num],
                [])
            create_form.display_name = 'Test Authorization ' + str(num) + ' (override)'
            create_form.description = 'Test Authorization for AuthorizationSession tests'
        ...
        # Set up ResourceRelationship search authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_RESOURCE_RELATIONSHIP_FUNCTION_ID,
    ...
    def class_tear_down():
        if not is_never_authz(request.cls.service_config):
            for catalog in request.cls.resource_mgr.get_bins():
                for obj in catalog.get_resource_relationships():
                    catalog.delete_resource_relationship(obj.ident)
