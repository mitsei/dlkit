"""TestAuthZ implementations of assessment.AssessmentOffered"""
...
LOOKUP_ASSESSMENT_OFFERED_FUNCTION_ID = Id(**{'identifier': 'lookup', 'namespace': 'assessment.AssessmentOffered', 'authority': 'ODL.MIT.EDU'})
SEARCH_ASSESSMENT_OFFERED_FUNCTION_ID = Id(**{'identifier': 'search', 'namespace': 'assessment.AssessmentOffered', 'authority': 'ODL.MIT.EDU'})
CREATE_ASSESSMENT_OFFERED_FUNCTION_ID = Id(**{'identifier': 'create', 'namespace': 'assessment.AssessmentOffered', 'authority': 'ODL.MIT.EDU'})
DELETE_ASSESSMENT_OFFERED_FUNCTION_ID = Id(**{'identifier': 'delete', 'namespace': 'assessment.AssessmentOffered', 'authority': 'ODL.MIT.EDU'})
ASSIGN_ASSESSMENT_OFFERED_FUNCTION_ID = Id(**{'identifier': 'assign', 'namespace': 'assessment.AssessmentOfferedBank', 'authority': 'ODL.MIT.EDU'})
...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        #       |        |        |
        #       3        4       5(t)      6     7(t)   (the 'blue' assessment_offered in bank 2 is also assigned to bank 7)
        ...
        # Set up AssessmentOffered lookup authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                LOOKUP_ASSESSMENT_OFFERED_FUNCTION_ID,
        ...
        # Set up AssessmentOffered lookup override authorizations for Jane
        for num in [7]:
        ...
        # Set up AssessmentOffered search override authorizations for Jane
        for num in [7]:
            create_form = request.cls.override_authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_ASSESSMENT_OFFERED_FUNCTION_ID,
                request.cls.bank_id_list[num],
                [])
            create_form.display_name = 'Test Authorization ' + str(num) + ' (override)'
            create_form.description = 'Test Authorization for AuthorizationSession tests'
        ...
        # Set up AssessmentOffered search authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_ASSESSMENT_OFFERED_FUNCTION_ID,
...
class TestAssessmentOfferedAuthzAdapter(object):
    ...
    def test_lookup_bank_0_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            #     assessments_offered = bank.get_assessments_offered()
            # with pytest.raises(errors.NotFound):
            #     assessments_offered = bank.get_assessments_offered_by_genus_type(BLUE_TYPE)
            # for assessment_offered_id in self.assessment_offered_id_lists[0]:
            #     with pytest.raises(errors.NotFound):
            #         assessment_offered = bank.get_assessment_offered(assessment_offered_id)
            # with pytest.raises(errors.NotFound):
            #     assessments_offered = bank.get_assessments_offered_by_ids(self.assessment_offered_id_lists[0])
    ...
    def test_lookup_bank_0_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_plenary_assessment_offered_view()
            assert bank.can_lookup_assessments_offered()
            assert bank.get_assessments_offered().available() == 1
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 1
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).next().ident == self.assessment_offered_id_lists[2][1]
            bank.get_assessment_offered(self.assessment_offered_id_lists[2][1])
            for assessment_offered_num in [0, 2]:
                with pytest.raises(errors.NotFound):  # Is this right?  Perhaps PermissionDenied
                    assessment_offered = bank.get_assessment_offered(self.assessment_offered_id_lists[2][assessment_offered_num])
    ...
    def test_lookup_bank_0_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            # print "START"
            assert bank.get_assessments_offered().available() == 13
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 5
            for assessment_offered in bank.get_assessments_offered():
                bank.get_assessment_offered(assessment_offered.ident)
            assessment_offered_ids = [assessment_offered.ident for assessment_offered in bank.get_assessments_offered()]
            bank.get_assessments_offered_by_ids(assessment_offered_ids)
            for assessment_offered_id in self.assessment_offered_id_lists[0]:
                with pytest.raises(errors.NotFound):
                    assessment_offered = bank.get_assessment_offered(assessment_offered_id)
            assessment_offered = bank.get_assessment_offered(self.assessment_offered_id_lists[2][1])
            for assessment_offered_num in [0, 2]:
                with pytest.raises(errors.NotFound):
                    assessment_offered = bank.get_assessment_offered(self.assessment_offered_id_lists[2][assessment_offered_num])
            for assessment_offered_id in self.assessment_offered_id_lists[1]:
                    assessment_offered = bank.get_assessment_offered(assessment_offered_id)
            for assessment_offered_id in self.assessment_offered_id_lists[3]:
                    assessment_offered = bank.get_assessment_offered(assessment_offered_id)
            for assessment_offered_id in self.assessment_offered_id_lists[4]:
                    assessment_offered = bank.get_assessment_offered(assessment_offered_id)
            for assessment_offered_id in self.assessment_offered_id_lists[5]:
                    assessment_offered = bank.get_assessment_offered(assessment_offered_id)
    ...
    def test_lookup_bank_0_comparative_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_isolated_bank_view()
            bank.use_comparative_assessment_offered_view()
            assert bank.get_assessments_offered().available() == 0
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 0
    ...
    def test_lookup_bank_1_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_isolated_bank_view()
            bank.use_plenary_assessment_offered_view()
            assert bank.get_assessments_offered().available() == 3
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 1
    ...
    def test_lookup_bank_1_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_federated_bank_view()
            bank.use_plenary_assessment_offered_view()
            assert bank.get_assessments_offered().available() == 9
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 3
    ...
    def test_lookup_bank_2_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_federated_bank_view()
            bank.use_comparative_assessment_offered_view()
            assert bank.get_assessments_offered().available() == 4
            assert bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available() == 2
            # self.assertEqual(bank.get_assessments_offered().available(), 3)
            # self.assertEqual(bank.get_assessments_offered_by_genus_type(BLUE_TYPE).available(), 1)
    ...
    def test_query_bank_0_federated(self):
        if not is_never_authz(self.service_config):
            ...
            assert bank.get_assessments_offered_by_query(query).available() == 8
            query.clear_display_name_terms()
            query.match_display_name('blue')
            assert bank.get_assessments_offered_by_query(query).available() == 5
    ...
    def test_query_bank_1_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            query = bank.get_assessment_offered_query()
            query.match_display_name('red')
            assert bank.get_assessments_offered_by_query(query).available() == 2
    ...
    def test_query_bank_1_federated(self):
        if not is_never_authz(self.service_config):
            ...
            query = bank.get_assessment_offered_query()
            query.match_display_name('red')
            assert bank.get_assessments_offered_by_query(query).available() == 6
