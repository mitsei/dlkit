"""TestAuthZ implementations of assessment.AssessmentTaken"""
...
LOOKUP_ASSESSMENT_TAKEN_FUNCTION_ID = Id(**{'identifier': 'lookup', 'namespace': 'assessment.AssessmentTaken', 'authority': 'ODL.MIT.EDU'})
SEARCH_ASSESSMENT_TAKEN_FUNCTION_ID = Id(**{'identifier': 'search', 'namespace': 'assessment.AssessmentTaken', 'authority': 'ODL.MIT.EDU'})
CREATE_ASSESSMENT_TAKEN_FUNCTION_ID = Id(**{'identifier': 'create', 'namespace': 'assessment.AssessmentTaken', 'authority': 'ODL.MIT.EDU'})
DELETE_ASSESSMENT_TAKEN_FUNCTION_ID = Id(**{'identifier': 'delete', 'namespace': 'assessment.AssessmentTaken', 'authority': 'ODL.MIT.EDU'})
ASSIGN_ASSESSMENT_TAKEN_FUNCTION_ID = Id(**{'identifier': 'assign', 'namespace': 'assessment.AssessmentTakenBank', 'authority': 'ODL.MIT.EDU'})
...
def authz_adapter_class_fixture(request):
    ...
    if not is_never_authz(request.cls.service_config):
        ...
        #       |        |        |
        #       3        4       5(t)      6     7(t)   (the 'blue' assessment_taken in bank 2 is also assigned to bank 7)
        ...
        # Set up AssessmentTaken lookup authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                LOOKUP_ASSESSMENT_TAKEN_FUNCTION_ID,
        ...
        # Set up AssessmentTaken lookup override authorizations for Jane
        for num in [7]:
        ...
        # Set up AssessmentTaken search override authorizations for Jane
        for num in [7]:
            create_form = request.cls.override_authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_ASSESSMENT_TAKEN_FUNCTION_ID,
                request.cls.bank_id_list[num],
                [])
            create_form.display_name = 'Test Authorization ' + str(num) + ' (override)'
            create_form.description = 'Test Authorization for AuthorizationSession tests'
        ...
        # Set up AssessmentTaken search authorizations for Jane
        for num in [1, 5]:
            create_form = request.cls.authz_admin_session.get_authorization_form_for_create_for_agent(
                AGENT_ID,
                SEARCH_ASSESSMENT_TAKEN_FUNCTION_ID,
...
def authz_adapter_test_fixture(request):
    request.cls.assessment_taken_id_lists = []
    count = 0
    if not is_never_authz(request.cls.service_config):
        ...
        for bank_ in request.cls.bank_list:
            request.cls.assessment_taken_id_lists.append([])
            for color in ['Red', 'Blue', 'Red']:
                create_form = bank_.get_assessment_taken_form_for_create(request.cls.assessment_offered.ident, [])
                create_form.display_name = color + ' ' + str(count) + ' AssessmentTaken'
                create_form.description = color + ' assessment_taken for authz adapter tests from Bank number ' + str(count)
                if color == 'Blue':
                    create_form.genus_type = BLUE_TYPE
                assessment_taken = bank_.create_assessment_taken(create_form)
                if count == 2 and color == 'Blue':
                    request.cls.assessment_mgr.assign_assessment_taken_to_bank(
                        assessment_taken.ident,
                        request.cls.bank_id_list[7])
                request.cls.assessment_taken_id_lists[count].append(assessment_taken.ident)
            count += 1

    def test_tear_down():
        if not is_never_authz(request.cls.service_config):
            for index, bank_ in enumerate(request.cls.bank_list):
                for assessment_taken_id in request.cls.assessment_taken_id_lists[index]:
                    bank_.delete_assessment_taken(assessment_taken_id)
...
class TestAssessmentTakenAuthzAdapter(object):
    ...
    def test_lookup_bank_0_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            #     assessments_taken = bank.get_assessments_taken()
            # with pytest.raises(errors.NotFound):
            #     assessments_taken = bank.get_assessments_taken_by_genus_type(BLUE_TYPE)
            # for assessment_taken_id in self.assessment_taken_id_lists[0]:
            #     with pytest.raises(errors.NotFound):
            #         assessment_taken = bank.get_assessment_taken(assessment_taken_id)
            # with pytest.raises(errors.NotFound):
            #     assessments_taken = bank.get_assessments_taken_by_ids(self.assessment_taken_id_lists[0])
    ...
    def test_lookup_bank_0_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_plenary_assessment_taken_view()
            assert bank.can_lookup_assessments_taken()
            assert bank.get_assessments_taken().available() == 1
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 1
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).next().ident == self.assessment_taken_id_lists[2][1]
            bank.get_assessment_taken(self.assessment_taken_id_lists[2][1])
            for assessment_taken_num in [0, 2]:
                with pytest.raises(errors.NotFound):  # Is this right?  Perhaps PermissionDenied
                    assessment_taken = bank.get_assessment_taken(self.assessment_taken_id_lists[2][assessment_taken_num])
    ...
    def test_lookup_bank_0_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            # print "START"
            assert bank.get_assessments_taken().available() == 13
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 5
            for assessment_taken in bank.get_assessments_taken():
                bank.get_assessment_taken(assessment_taken.ident)
            assessment_taken_ids = [assessment_taken.ident for assessment_taken in bank.get_assessments_taken()]
            bank.get_assessments_taken_by_ids(assessment_taken_ids)
            for assessment_taken_id in self.assessment_taken_id_lists[0]:
                with pytest.raises(errors.NotFound):
                    assessment_taken = bank.get_assessment_taken(assessment_taken_id)
            assessment_taken = bank.get_assessment_taken(self.assessment_taken_id_lists[2][1])
            for assessment_taken_num in [0, 2]:
                with pytest.raises(errors.NotFound):
                    assessment_taken = bank.get_assessment_taken(self.assessment_taken_id_lists[2][assessment_taken_num])
            for assessment_taken_id in self.assessment_taken_id_lists[1]:
                    assessment_taken = bank.get_assessment_taken(assessment_taken_id)
            for assessment_taken_id in self.assessment_taken_id_lists[3]:
                    assessment_taken = bank.get_assessment_taken(assessment_taken_id)
            for assessment_taken_id in self.assessment_taken_id_lists[4]:
                    assessment_taken = bank.get_assessment_taken(assessment_taken_id)
            for assessment_taken_id in self.assessment_taken_id_lists[5]:
                    assessment_taken = bank.get_assessment_taken(assessment_taken_id)
    ...
    def test_lookup_bank_0_comparative_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_isolated_bank_view()
            bank.use_comparative_assessment_taken_view()
            assert bank.get_assessments_taken().available() == 0
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 0
    ...
    def test_lookup_bank_1_plenary_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_isolated_bank_view()
            bank.use_plenary_assessment_taken_view()
            assert bank.get_assessments_taken().available() == 3
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 1
    ...
    def test_lookup_bank_1_plenary_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_federated_bank_view()
            bank.use_plenary_assessment_taken_view()
            assert bank.get_assessments_taken().available() == 9
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 3
    ...
    def test_lookup_bank_2_comparative_federated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_federated_bank_view()
            bank.use_comparative_assessment_taken_view()
            assert bank.get_assessments_taken().available() == 4
            assert bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available() == 2
            # self.assertEqual(bank.get_assessments_taken().available(), 3)
            # self.assertEqual(bank.get_assessments_taken_by_genus_type(BLUE_TYPE).available(), 1)
    ...
    def test_query_bank_0_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            bank.use_isolated_bank_view()
            with pytest.raises(errors.PermissionDenied):
                query = bank.get_assessment_taken_query()
    ...
    def test_query_bank_0_federated(self):
        if not is_never_authz(self.service_config):
            ...
            assert bank.get_assessments_taken_by_query(query).available() == 8
            query.clear_display_name_terms()
            query.match_display_name('blue')
            assert bank.get_assessments_taken_by_query(query).available() == 5
    ...
    def test_query_bank_1_isolated(self):
        if not is_never_authz(self.service_config):
            ...
            query = bank.get_assessment_taken_query()
            query.match_display_name('red')
            assert bank.get_assessments_taken_by_query(query).available() == 2
    ...
    def test_query_bank_1_federated(self):
        if not is_never_authz(self.service_config):
            ...
            query = bank.get_assessment_taken_query()
            query.match_display_name('red')
            assert bank.get_assessments_taken_by_query(query).available() == 6
