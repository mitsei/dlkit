"""Unit tests of learning records."""

import unittest




class TestObjectiveRecord(unittest.TestCase):
    """Tests for ObjectiveRecord"""






class TestObjectiveQueryRecord(unittest.TestCase):
    """Tests for ObjectiveQueryRecord"""






class TestObjectiveFormRecord(unittest.TestCase):
    """Tests for ObjectiveFormRecord"""






class TestObjectiveSearchRecord(unittest.TestCase):
    """Tests for ObjectiveSearchRecord"""






class TestActivityRecord(unittest.TestCase):
    """Tests for ActivityRecord"""






class TestActivityQueryRecord(unittest.TestCase):
    """Tests for ActivityQueryRecord"""






class TestActivityFormRecord(unittest.TestCase):
    """Tests for ActivityFormRecord"""






class TestActivitySearchRecord(unittest.TestCase):
    """Tests for ActivitySearchRecord"""






class TestProficiencyRecord(unittest.TestCase):
    """Tests for ProficiencyRecord"""






class TestProficiencyQueryRecord(unittest.TestCase):
    """Tests for ProficiencyQueryRecord"""






class TestProficiencyFormRecord(unittest.TestCase):
    """Tests for ProficiencyFormRecord"""






class TestProficiencySearchRecord(unittest.TestCase):
    """Tests for ProficiencySearchRecord"""






class TestObjectiveBankRecord(unittest.TestCase):
    """Tests for ObjectiveBankRecord"""






class TestObjectiveBankQueryRecord(unittest.TestCase):
    """Tests for ObjectiveBankQueryRecord"""






class TestObjectiveBankFormRecord(unittest.TestCase):
    """Tests for ObjectiveBankFormRecord"""






class TestObjectiveBankSearchRecord(unittest.TestCase):
    """Tests for ObjectiveBankSearchRecord"""




