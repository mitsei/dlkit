...
from .objects import Asset, AssetList, AssetContent, AssetContentForm, AssetContentList
...
class AssetAdminSession(abc_repository_sessions.AssetAdminSession, osid_sessions.OsidSession):
    ...
    def get_asset_content_form_for_create(self,
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        if FILESYSTEM_ASSET_CONTENT_RECORD_TYPE in asset_content_record_types:
            asset_content_record_types.remove(FILESYSTEM_ASSET_CONTENT_RECORD_TYPE)
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        try:
            if asset_content.has_url() and 'repository/AssetContent' in asset_content.get_url():
                # Because we want to keep the assetContent file matching with the
                # actual assetContentId, here we update the assetContent
                # and the file saved on disk.
                self._update_asset_content_filename_on_disk_to_match_id(asset_content)
                asset_content = self._update_asset_content_url_to_match_id(asset_content)
    ...
    def get_asset_content_form_for_update(self, asset_content_id=None):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        asset_content_form = self._provider_session.get_asset_content_form_for_update(
            asset_content_id)
        if 'repository/AssetContent' in asset_content_form.get_url_metadata().get_existing_string_values():
    ...
    def update_asset_content(self, asset_content_form=None):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if asset_content is not None and asset_content.has_url() and \
                'repository/AssetContent' in asset_content.get_url():
            return AssetContent(asset_content, self._config_map)
        return asset_content
