...
class ObjectiveLookupSession(abc_learning_sessions.ObjectiveLookupSession, osid_sessions.OsidSession):
    ...
    def __init__(self, objective_bank_id=None, proxy=None):
        ...
        self._objective_bank_identifier = self._objective_bank_id.get_identifier()
        self._credential = None
        if proxy is not None and proxy.has_authentication():
            authentication = proxy.get_authentication()
            # I LEFT OFF HERE
    ...
    def get_objective(self, objective_id=None):
        ...
        if objective_id is None:
            raise NullArgument()
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier +
                    '/objectives/' + objective_id.get_identifier())
    ...
    def get_objectives_by_genus_type(self, objective_genus_type=None):
        ...
        if objective_genus_type is None:
            raise NullArgument()
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives?genustypeid=' +
                    objective_genus_type.get_identifier())
    ...
    def get_objectives(self):
        ...
        """
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives')
        return objects.ObjectiveList(self._get_request(url_path))
...
class ObjectiveQuerySession(abc_learning_sessions.ObjectiveQuerySession, osid_sessions.OsidSession):
    ...
    """
    def __init__(self, objective_bank_id):
        self._objective_view = COMPARATIVE
        self._objective_bank_view = FEDERATED
        self._objective_bank_id = objective_bank_id
        self._objective_bank_identifier = objective_bank_id.get_identifier()
...
class ObjectiveAdminSession(abc_learning_sessions.ObjectiveAdminSession, osid_sessions.OsidSession):
    ...
    """
    def __init__(self, objective_bank_id=None):
        if objective_bank_id is None:
            # For now we are just going to find the first objective bank
            # This needs to change!
            url_path = '/handcar/services/learning/objectivebanks'
    ...
    def create_objective(self, objective_form=None):
        ...
        raise:  IllegalState - objectiveForm already used in a create
                transaction
        raise:  INVALID_ARGUMENT - one or more of the form elements is
                invalid
        ...
        if not objective_form.is_valid():
            raise INVALID_ARGUMENT('one or more of the form elements is invalid')
    ...
    def get_objective_form_for_update(self, objective_id=None):
        ...
        if objective_id is None:
            raise NullArgument()
        try:
            objective = ObjectiveLookupSession(self._objective_bank_id).get_objective(objective_id)
        except Exception:
            raise
...
class ObjectiveHierarchySession(abc_learning_sessions.ObjectiveHierarchySession, osid_sessions.OsidSession):
    ...
    def get_root_objective_ids(self):
        ...
        """
        url_path = (self._base_path + '/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/rootids')
        id_list = list()
        for identifier in self._get_request(url_path)['ids']:
            id_list.append(Id(authority=settings.AUTHORITY,
                              namespace='learning.Objective',
    ...
    def get_root_objectives(self):
        ...
        """
        url_path = (self._base_path + '/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/roots?descendentlevels=0')
        return objects.ObjectiveList(self._get_request(url_path))
    ...
    def get_parent_objective_ids(self, objective_id=None):
        ...
        url_path = (self._base_path + '/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/parentids')
        id_list = list()
        for identifier in self._get_request(url_path)['ids']:
            id_list.append(Id(authority=settings.AUTHORITY,
                              namespace='learning.Objective',
    ...
    def get_parent_objectives(self, objective_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/parents')
        return objects.ObjectiveList(self._get_request(url_path))
    ...
    def get_child_objective_ids(self, objective_id=None):
        ...
        url_path = (self._base_path + '/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/childids')
        id_list = list()
        for identifier in self._get_request(url_path)['ids']:
            id_list.append(Id(authority=settings.AUTHORITY,
                              namespace='learning.Objective',
    ...
    def get_child_objectives(self, objective_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/children')
        return objects.ObjectiveList(self._get_request(url_path))
    ...
    def get_objective_node_ids(self, objective_id=None, ancestor_levels=None, descendant_levels=None, include_siblings=None):
    ...
    def get_objective_nodes(self, objective_id=None, ancestor_levels=None, descendant_levels=None, include_siblings=None):
        """Gets a portion of the hierarchy for the given objective.
...
class ObjectiveHierarchyDesignSession(abc_learning_sessions.ObjectiveHierarchyDesignSession, osid_sessions.OsidSession):
    ...
    def add_child_objective(self, objective_id=None, child_id=None):
        ...
        for ident in ohs.get_child_objective_ids(objective_id):
            ids_arg['ids'].append(ident.get_identifier())
        ids_arg['ids'].append(child_id.get_identifier())
    ...
    def remove_child_objectives(self, objective_id=None):
        ...
        if objective_id is None:
            raise NullArgument()
        ols = ObjectiveLookupSession(self._objective_bank_id)
        try:
...
class ObjectiveSequencingSession(abc_learning_sessions.ObjectiveSequencingSession, osid_sessions.OsidSession):
    ...
    def move_objective_ahead(self, parent_objective_id=None, reference_objective_id=None, objective_id=None):
        ...
        if objective_id != reference_objective_id:
            ids_arg['ids'].remove(objective_id.get_identifier())
            index = ids_arg['ids'].index(reference_objective_id.get_identifier())
            ids_arg['ids'].insert(index, objective_id.get_identifier())

        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    parent_objective_id.get_identifier() + '/childids')
        try:
            result = self._put_request(url_path, ids_arg)
        except Exception:
            raise
    ...
    def move_objective_behind(self, parent_objective_id=None, reference_objective_id=None, objective_id=None):
        ...
        if objective_id != reference_objective_id:
            ids_arg['ids'].remove(objective_id.get_identifier())
            index = ids_arg['ids'].index(reference_objective_id.get_identifier())
            ids_arg['ids'].insert(index + 1, objective_id.get_identifier())
...
class ObjectiveRequisiteSession(abc_learning_sessions.ObjectiveRequisiteSession, osid_sessions.OsidSession):
    ...
    def get_requisite_objectives(self, objective_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/requisites')
        return objects.ObjectiveList(self._get_request(url_path))
    ...
    def get_dependent_objectives(self, objective_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/dependents')
        return objects.ObjectiveList(self._get_request(url_path))
    ...
    def get_equivalent_objectives(self, objective_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/equivalents')
        return objects.ObjectiveList(self._get_request(url_path))
...
class ObjectiveRequisiteAssignmentSession(abc_learning_sessions.ObjectiveRequisiteAssignmentSession, osid_sessions.OsidSession):
    ...
    def assign_objective_requisite(self, objective_id=None, requisite_objective_id=None):
        ...
        if objective_id is None or requisite_objective_id is None:
            raise NullArgument()
        ors = ObjectiveRequisiteSession(self._objective_bank_id)
        ids_arg = {'ids': []}
        for objective in ors.get_requisite_objectives(objective_id):
            if objective.get_id() == requisite_objective_id:
                raise AlreadyExists()
            ids_arg['ids'].append(objective.get_id().get_identifier())
        ids_arg['ids'].append(requisite_objective_id.get_identifier())

        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/objectives/' +
                    objective_id.get_identifier() + '/requisiteids')
        try:
            result = self._put_request(url_path, ids_arg)
        except Exception:
            raise
    ...
    def unassign_objective_requisite(self, objective_id=None, requisite_objective_id=None):
        ...
        for objective in ors.get_requisite_objectives(objective_id):
            ids_arg['ids'].append(objective.get_id().get_identifier())
        if requisite_objective_id.get_identifier() not in ids_arg['ids']:
            raise NotFound()
        ids_arg['ids'].remove(requisite_objective_id.get_identifier())
...
class ActivityLookupSession(abc_learning_sessions.ActivityLookupSession, osid_sessions.OsidSession):
    ...
    def get_activity(self, activity_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier +
                    '/activities/' + activity_id.get_identifier())
    ...
    def get_activities_by_ids(self, activity_ids=None):
        ...
        return objects.ActivityList(activity)
    ...
    def get_activities_by_genus_type(self, activity_genus_type=None):
        ...
        if activity_genus_type is None:
            raise NullArgument()
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/activities?genustypeid=' +
                    activity_genus_type.get_identifier())
    ...
    def get_activities_for_objective(self, objective_id=None):
        ...
        for activity in self.get_activities():
            if activity.get_objective_id() == objective_id:
    ...
    def get_activities(self):
        ...
        """
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    self._objective_bank_identifier + '/activities')
        return objects.ActivityList(self._get_request(url_path))
...
class ActivityAdminSession(abc_learning_sessions.ActivityAdminSession, osid_sessions.OsidSession):
    ...
    def get_activity_form_for_update(self, activity_id=None):
        ...
        if activity_id is None:
            raise NullArgument()
        try:
            activity = ActivityLookupSession().get_activity(activity_id)
        except Exception:
            raise
    ...
    def update_activity(self, activity_form=None):
        ...
        raise:  IllegalState - activityForm already used in an update
                transaction
        raise:  INVALID_ARGUMENT - the form contains an invalid value
        raise:  NullArgument - activityForm is null
        raise:  OperationFailed - unable to complete request
        raise:  PermissionDenied - authorization failure
        raise:  Unsupported - activityForm did not originate from
...
class ObjectiveBankLookupSession(abc_learning_sessions.ObjectiveBankLookupSession, osid_sessions.OsidSession):
    ...
    def get_objective_bank(self, objective_bank_id=None):
        ...
        if objective_bank_id is None:
            raise NullArgument()
        url_path = '/handcar/services/learning/objectivebanks/' + objective_bank_id.get_identifier()
    ...
    def get_objective_banks_by_ids(self, objective_bank_ids=None):
        ...
        for i in objective_bank_ids:
            bank = None
            url_path = '/handcar/services/learning/objectivebanks/' + i.get_identifier()
            try:
    ...
    def get_objective_banks_by_genus_type(self, objective_bank_genus_type=None):
        ...
        url_path = '/handcar/services/learning/objectivebanks'
        banks_of_type = []
        all_banks = self._get_request(url_path)
        for bank in all_banks:
            # DO WE NEED TO CHECK ALL THREE ATRIBUTES OF THE Id HERE?
            if bank['genusTypeId'] == objective_bank_genus_type.get_identifier():
                banks_of_type.append[bank]
        return objects.ObjectiveBankList(banks_of_type)
...
class ObjectiveBankAdminSession(abc_learning_sessions.ObjectiveBankAdminSession, osid_sessions.OsidSession):
    ...
    def get_objective_bank_form_for_update(self, objective_bank_id=None):
        ...
        if objective_bank_id is None:
            raise NullArgument()
        try:
            objective_bank = ObjectiveBankLookupSession().get_objective_bank(objective_bank_id)
        except Exception:
            raise
    ...
    def update_objective_bank(self, objective_bank_id=None, objective_bank_form=None):
        ...
        """
        # NOTE: Not really using objective_bank_id since its part of the form.
        #   Is that correct?  Not the same pattern as ObjectiveForm or ActivityForm
        if objective_bank_form is None or objective_bank_id is None:
            raise NullArgument()
        if not isinstance(objective_bank_form, abc_learning_objects.ObjectiveBankForm):
            raise InvalidArgument('argument type is not an ObjectiveBankForm')
    ...
    def delete_objective_bank(self, objective_bank_id=None):
        ...
        url_path = ('/handcar/services/learning/objectivebanks/' +
                    objective_bank_id.get_identifier())
        result = self._delete_request(url_path)
        return objects.ObjectiveBank(result)  # Not expected to return anything
...
class ObjectiveBankHierarchySession(abc_learning_sessions.ObjectiveBankHierarchySession, osid_sessions.OsidSession):
    ...
    def get_objective_bank_node_ids(self, objective_bank_id=None, ancestor_levels=None, descendant_levels=None, include_siblings=None):
