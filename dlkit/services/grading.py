...
class GradingProxyManager(osid.OsidProxyManager, GradingProfile, GradingManager, grading_managers.GradingProxyManager):
...
# The following methods are from osid.grading.GradeSystemLookupSession
    ...
    def get_gradebook_id(self):
        """Pass through to provider GradeSystemLookupSession.get_gradebook_id"""
        # Built from: templates/osid_session.GenericObjectLookupSession.get_catalog_id
        return self._get_provider_session('grade_system_lookup_session').get_gradebook_id()

    gradebook_id = property(fget=get_gradebook_id)

    def get_gradebook(self):
        """Pass through to provider GradeSystemLookupSession.get_gradebook"""
        # Built from: templates/osid_session.GenericObjectLookupSession.get_catalog
        return Gradebook(
            self._provider_manager,
            self._get_provider_session('grade_system_lookup_session').get_gradebook(*args, **kwargs),
            self._runtime,
            self._proxy)
...
# The following methods are from osid.grading.GradeEntryLookupSession
    ...
    def get_grade_entries_on_date(self, *args, **kwargs):
        """Pass through to provider GradeEntryLookupSession.get_grade_entries_on_date"""
        # Built from: templates/osid_session.GenericRelationshipLookupSession.get_relationships_on_date
        return self._get_provider_session('grade_entry_lookup_session').get_grade_entries_on_date(*args, **kwargs)
    ...
    def get_grade_entries_for_gradebook_column_and_resource(self, *args, **kwargs):
        """Pass through to provider GradeEntryLookupSession.get_grade_entries_for_gradebook_column_and_resource"""
        # Built from: templates/osid_session.GenericRelationshipLookupSession.get_relationships_for_peers
        return self._get_provider_session('grade_entry_lookup_session').get_grade_entries_for_gradebook_column_and_resource(*args, **kwargs)
...
# The following methods are from osid.grading.GradebookColumnAdminSession
    ...
    def delete_gradebook_column(self, *args, **kwargs):
        ...
        # Not templated -- check the hand-built implementations
        return self._get_provider_session('gradebook_column_admin_session').delete_gradebook_column(*args, **kwargs)
