"""JSON implementations of hierarchy sessions."""
...
class HierarchyLookupSession(abc_hierarchy_sessions.HierarchyLookupSession, osid_sessions.OsidSession):
    ...
    def get_hierarchy(self, hierarchy_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._catalog_session is not None:
            return self._catalog_session.get_catalog(catalog_id=hierarchy_id)
        collection = JSONClientValidated('hierarchy',
                                         collection='Hierarchy',
                                         runtime=self._runtime)
        # Need to consider how to best deal with the "phantom root" catalog issue
        if hierarchy_id.get_identifier() == PHANTOM_ROOT_IDENTIFIER:
            return self._get_phantom_root_catalog(cat_class=objects.Hierarchy, cat_name='Hierarchy')
        try:
            result = collection.find_one({'_id': ObjectId(self._get_id(hierarchy_id,
                                                                       'hierarchy').get_identifier())})
        except errors.NotFound:
        ...
        return objects.Hierarchy(osid_object_map=result, runtime=self._runtime, proxy=self._proxy)
    ...
    def get_hierarchies_by_ids(self, hierarchy_ids):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._catalog_session is not None:
            return self._catalog_session.get_catalogs_by_ids(catalog_ids=hierarchy_ids)
        catalog_id_list = []
        ...
        return objects.HierarchyList(result, runtime=self._runtime, proxy=self._proxy)
    ...
    def get_hierarchies_by_genus_type(self, hierarchy_genus_type):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        # NOTE: This implementation currently ignores plenary view
        if self._catalog_session is not None:
            return self._catalog_session.get_catalogs_by_genus_type(catalog_genus_type=hierarchy_genus_type)
        collection = JSONClientValidated('hierarchy',
                                         collection='Hierarchy',
                                         runtime=self._runtime)
        result = collection.find({"genusTypeId": str(hierarchy_genus_type)}).sort('_id', DESCENDING)
...
class HierarchyAdminSession(abc_hierarchy_sessions.HierarchyAdminSession, osid_sessions.OsidSession):
    ...
    @utilities.arguments_not_none
    def can_create_hierarchy_with_record_types(self):
    ...
    def get_hierarchy_form_for_create(self, hierarchy_record_types):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._catalog_session is not None:
            return self._catalog_session.get_catalog_form_for_create(catalog_record_types=hierarchy_record_types)
    ...
    def create_hierarchy(self, hierarchy_form):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._catalog_session is not None:
            return self._catalog_session.create_catalog(catalog_form=hierarchy_form)
        collection = JSONClientValidated('hierarchy',
                                         collection='Hierarchy',
                                         runtime=self._runtime)
    ...
    def get_hierarchy_form_for_update(self, hierarchy_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        if self._catalog_session is not None:
            return self._catalog_session.get_catalog_form_for_update(catalog_id=hierarchy_id)
        collection = JSONClientValidated('hierarchy',
                                         collection='Hierarchy',
                                         runtime=self._runtime)
