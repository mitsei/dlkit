...
class CatalogSession(abc_cataloging_sessions.CatalogSession, osid_sessions.OsidSession):
    ...
    def get_ids_by_catalog(self, catalog_id):
        ...
        return: (osid.id.IdList) - list of ``Ids`` mapped to the given
                ``catalog_id``
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        id_list = []
        for id in self.get_ids_by_catalog(catalog_id):
            id_list.append(id.get_id())
    ...
    def get_ids_by_catalogs(self, catalog_ids):
        ...
        arg:    catalog_ids (osid.id.IdList): an ``Id``
        return: (osid.id.IdList) - list of catalogs containing the given
                ``Id``
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        id_list = []
        for id in self.get_ids_by_catalogs(catalog_ids):
            id_list.append(id.get_id())
        return IdList(id_list)
    ...
    def get_catalog_ids_by_id(self, id_):
        ...
        return: (osid.id.IdList) - list of catalog ``Ids`` containing
                the given ``Id``
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        mgr = self._get_provider_manager('CATALOGING', local=True)
        lookup_session = mgr.get_id_lookup_session(proxy=self._proxy)
        lookup_session.use_federated_catalog_view()
        id = lookup_session.get_id(id_)
        id_list = []
        for idstr in id._my_map['assignedCatalogIds']:
    ...
    def get_catalogs_by_id(self, id_):
        ...
        arg:    id (osid.id.Id): an ``Id``
        return: (osid.cataloging.CatalogList) - list of catalogs
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        mgr = self._get_provider_manager('CATALOGING')
        lookup_session = mgr.get_catalog_lookup_session(proxy=self._proxy)
        return lookup_session.get_catalogs_by_ids(
            self.get_catalog_ids_by_id(id_))
...
class CatalogAssignmentSession(abc_cataloging_sessions.CatalogAssignmentSession, osid_sessions.OsidSession):
    ...
    def assign_id_to_catalog(self, id_, catalog_id):
        ...
        raise:  AlreadyExists - ``id`` is already mapped to
                ``catalog_id``
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        mgr = self._get_provider_manager('CATALOGING', local=True)
        lookup_session = mgr.get_catalog_lookup_session(proxy=self._proxy)
        lookup_session.get_catalog(catalog_id)  # to raise NotFound
        self._assign_object_to_catalog(id_, catalog_id)
    ...
    def unassign_id_from_catalog(self, id_, catalog_id):
        ...
        arg:    id (osid.id.Id): an ``Id``
        arg:    catalog_id (osid.id.Id): the ``Id`` of the ``Catalog``
        raise:  NotFound - ``id`` or ``catalog_id`` not found or ``id``
                is not mapped to ``catalog_id``
        raise:  NullArgument - ``id`` or ``catalog_id`` is ``null``
        raise:  OperationFailed - unable to complete request
        raise:  PermissionDenied - authorization failure
        *compliance: mandatory -- This method must be implemented.*
        ...
        mgr = self._get_provider_manager('CATALOGING', local=True)
        lookup_session = mgr.get_catalog_lookup_session(proxy=self._proxy)
        cat = lookup_session.get_catalog(catalog_id)  # to raise NotFound
        self._unassign_object_from_catalog(id_, catalog_id)
    ...
    def reassign_id_to_catalog(self, id_, from_catalog_id, to_catalog_id):
        ...
        arg:    id (osid.id.Id): the ``Id``
        arg:    from_catalog_id (osid.id.Id): the ``Id`` of the current
                ``Catalog``
        arg:    to_catalog_id (osid.id.Id): the ``Id`` of the
                destination ``Catalog``
        raise:  NotFound - ``id, from_catalog_id,`` or ``to_catalog_id``
                not found or ``id`` not mapped to ``from_catalog_id``
        raise:  NullArgument - ``id, from_catalog_id,`` or
                ``to_catalog_id`` is ``null``
        raise:  OperationFailed - unable to complete request
        raise:  PermissionDenied - authorization failure
        *compliance: mandatory -- This method must be implemented.*
        ...
        self.assign_id_to_catalog(id_, to_catalog_id)
        try:
            self.unassign_id_from_catalog(id_, from_catalog_id)
        except:  # something went wrong, roll back assignment to to_catalog_id
            self.unassign_id_from_catalog(id_, to_catalog_id)
            raise
